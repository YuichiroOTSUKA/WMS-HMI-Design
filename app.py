...
# =========================================================
# Demo assets = Gate Houses and gates inside (Gate House-level mode switching)
# =========================================================
...
def init_state():
    ...
    # --- Security/login (aligned to "login + access levels + activity logging + auto timeout")
    if "auth" not in ss:
        ss.auth = {
            "logged_in": False,
            "user": "operator",
            ...
            "idle_timeout_sec": 5 * 60,  # demo; adjust as needed
        }
    if "login_log" not in ss:
        ss.login_log = []  # list of dicts

    if "audit_log" not in ss:
        ss.audit_log = []  # actions / commands
    ...
    # --- Gate House type (TC/SPC) affects available modes (SPC: Remote Manual NOT supported)
    if "gatehouse_type" not in ss:
        # Map each gatehouse to TC/SPC (dummy assignment)
        ss.gatehouse_type = {}
        for stn, ghs in ASSETS.items():
            for gh in ghs.keys():
                # Make some SPC to demonstrate restriction
                ss.gatehouse_type[f"{stn}/{gh}"] = "SPC" if ("Ciberang" in gh or "Waru" in gh) else "TC"

    # --- Gate House-level mode (switching at gatehouse, not per gate)
    if "mode" not in ss:
        ss.mode = "REMOTE AUTOMATIC"  # LOCAL (LCP ACTIVE) / REMOTE MANUAL / REMOTE AUTOMATIC / REMOTE PROGRAM
    ...
    # --- Gate House (direction-level) process values: Qplan, Qact, Hplan, Hact, Ktarget, Kact
    if "gh_state" not in ss:
        ...
        for stn, ghs in ASSETS.items():
            for gh in ghs.keys():
                ...
                # demo "Ktarget from DSS" (can be 1.0..0.3 etc)
...
# =========================================================
# Core safety / interlock (simplified)
# =========================================================
def blocked() -> bool:
    ss = st.session_state
...
def send_cmd(cmd: str):
    ...
    # Record to each gate (gatehouse-level operation still updates "last cmd" per gate)
    ss = st.session_state
...
def set_gatehouse_open_pct(target_pct: int):
    """Spec: control all gates within one gatehouse to approximately same gate position."""
    touch_activity()
    ss = st.session_state
...
# =========================================================
# Remote Automatic logic (spec-aligned skeleton)
# - Ktarget vs Kact comparison (±5%)
# - Stop + alarm after 1 hour if cannot achieve
# - Gatehouse-level control, all gates same opening
# =========================================================
...
def auto_target_q(gh: dict) -> float:
    # Qtarget = Ktarget * Qplan
    return gh["k_target"] * gh["q_plan"]


def dummy_gate_opening_from_qtarget(q_target: float) -> int:
    # Placeholder for future: HQ curve + coefficients imported per location
    # Here we map Qtarget into an opening %
...
def apply_remote_automatic_if_running():
    ...
    gh["k_act"] = k_act  # store for UI

    diff_pct = (k_target - k_act) * 100.0  # as percentage points
    ...
    # If out-of-band persists too long -> stop + alarm
    ...
    # Target selection and gatehouse control (placeholder implementation)
    q_target = auto_target_q(gh)
    gp_target_pct = dummy_gate_opening_from_qtarget(q_target)

    # Control all gates in gatehouse to same opening (spec)
    step_all_gates_in_gatehouse(gp_target_pct)


# =========================================================
# Remote Program logic (spec-aligned)
# - K value pattern (A..I: 100..0)
# - Gate position mode (cm or %)
# - Drive time mode (minutes)
# =========================================================
K_PATTERNS = {
    "A (100%)": 1.00,
    "B (90%)": 0.90,
...
def apply_remote_program_if_running():
    ...
    # --- Program Mode 1: K value mode (Operator sets Ktarget instead of DSS)
    if ss.program_mode == "K VALUE":
        k_target = K_PATTERNS.get(ss.prog_k_pattern, 1.0)
        gh["k_target"] = k_target  # operator-defined here
        q_target = auto_target_q(gh)
        gp_target_pct = dummy_gate_opening_from_qtarget(q_target)
        step_all_gates_in_gatehouse(gp_target_pct)
        return

    # --- Program Mode 2: Gate position mode (Operator inputs Gp in cm or %)
    if ss.program_mode == "GATE POSITION":
        if ss.prog_gate_pos_unit == "%":
            target_pct = int(clamp(round(ss.prog_gate_pos_value), 0, 100))
        else:
            # "cm": convert to meters then to %
            # Use selected gate's max_open_m as representative (demo).
            # In real: gatehouse should enforce same max or handle each gate separately.
            rep = get_gate()
    ...
    # --- Program Mode 3: Drive time mode (Operator inputs minutes)
    if ss.program_mode == "DRIVE TIME":
        minutes = clamp(ss.prog_drive_minutes, 0.0, 30.0)
        delta_m = minutes * GATE_SPEED_M_PER_MIN

        # Apply per gate: convert current opening to m, +/- delta, back to %
        for g in all_gates_in_gatehouse():
            key = f"{ss.station}/{ss.gatehouse}/{g}"
...
def tick_gatehouse_signals():
    ...
    # Update actual Q (nudged toward target when auto/program is running)
    ...
    # Tie Hact loosely to Hplan + noise
...
# =========================================================
# SVG: Gate House overview schematic (no external images)
# =========================================================
...
# =========================================================
# Gate House Controls (mode-dependent, gatehouse-level)
# =========================================================
...
if mode == "REMOTE AUTOMATIC":
    card_start("Automatic Mode Control", "Gate House-level execution controls. Ktarget vs Kact (±5%) + 1-hour stop condition.", "🤖")
    ...
    with b1:
        if st.button("▶ Start", use_container_width=True, disabled=is_blocked):
            ...
            st.session_state.auto_first_exec_ts = None  # reset timer on (re)start
    ...
    diverging_bar(-dev_pct, scale_pct=10.0)  # visual: left=below target, right=above
...
# --- Remote Program controls (3 program sub-modes)
if mode == "REMOTE PROGRAM":
    ...
    if st.session_state.program_mode == "K VALUE":
        st.session_state.prog_k_pattern = st.selectbox("K Pattern (A–I)", list(K_PATTERNS.keys()), index=list(K_PATTERNS.keys()).index(st.session_state.prog_k_pattern))
    ...
    row(
        "Program state",
        "RUNNING" if st.session_state.program_running else "STOPPED",
        None,
        "hmi-ok" if st.session_state.program_running else "hmi-bad",
    )
    card_end()
    st.markdown("")

# --- Remote Manual controls (TC only)
if mode == "REMOTE MANUAL":
    ...
    if get_gatehouse_type() == "SPC":
        ...
        st.caption("Note: Although you can select a gate for viewing, manual action is applied as Gate House control in this demo.")
    card_end()
    st.markdown("")

# =========================================================
# Gate House Overview (schematic + Ktarget/Kact indicator)
# =========================================================
...
def panel_gate_status_and_controls():
    ...
    # Remote Manual: intuitive control (applies to gatehouse)
    ...
    st.selectbox("CCTV Camera", ["CCTV — Gate Area", "CCTV — Upstream", "CCTV — Downstream"], key="cctv_camera", label_visibility="collapsed")
